...
function EditorFields({ block, responseDefinition }: { block: Block; responseDefinition?: ResponseDefinition }) {
  ...
  if (definition.editorKey === "pdf_reader") return <><label className="is-wide">Display title (optional)<input name="title" defaultValue={value(config, "title")} maxLength={200}/></label><label className="is-wide">Description (optional)<textarea name="description" defaultValue={value(config, "description")} maxLength={3000} rows={3}/></label><label>Reading mode<select name="reader_mode" defaultValue={value(config, "readerMode") || "reader"}><option value="reader">Reader / Book</option><option value="slides">Slides</option><option value="fit_width">Fit Width</option></select><span className="admin-field-note">Slides adds left/right arrow-key navigation. All modes keep the PDF inline.</span></label></>;
