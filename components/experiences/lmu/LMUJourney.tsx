    ...
    <>
      ...
      <ol className="module-list">
        {discoveryModules.map((module, index) => {
          ...
          return <ModuleJourneyItem allowLockedNavigation={LMU_DEV_UNLOCK_ALL} started={storedProgress.some((item) => item.moduleId === module.id)} futureDistance={status === "locked" ? index - currentIndex : undefined} index={index} key={module.id} module={module} status={status} />;
        })}
        {resultModule && <ModuleJourneyItem allowLockedNavigation={LMU_DEV_UNLOCK_ALL} completed={progress.completed} index={10} key={resultModule.id} module={resultModule} status={progress.completed === progress.total ? "available" : "locked"} total={progress.total} />}
      </ol>
      {LMU_DEV_UNLOCK_ALL ? <div className="journey-dev-tools"><span>DEV: ALL SECTIONS UNLOCKED</span></div> : null}
    </>
  );
}
