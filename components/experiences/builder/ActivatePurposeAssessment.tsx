...
export function ActivatePurposeAssessment({ initialData, route, preview = false }: { initialData: unknown; route: Route; preview?: boolean }) {
  ...
  function select(answer: ActivatePurposeAnswer) {
    ...
    advanceTimer.current = setTimeout(() => {
      if (questionIndex === ACTIVATE_PURPOSE_QUESTIONS.length - 1 && activatePurposeComplete(next)) setShowResults(true);
  ...
  if (showResults && results) return <section aria-label="Activate Your Purpose Assessment results" className="purpose-assessment purpose-results-view">
  ...
  return <section aria-label="Activate Your Purpose Assessment" className="purpose-assessment">
