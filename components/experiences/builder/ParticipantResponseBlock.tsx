...
export function ParticipantResponseBlock({ context, route, configuration, responseKind, preview = false, allowFinalizedEditing = false }: { context: ParticipantResponseContext; route: Route; configuration: Record<string, unknown>; responseKind: ResponseKind; preview?: boolean; allowFinalizedEditing?: boolean }) {
  const finalized = context.response?.status === "submitted" || context.response?.status === "finalized";
  ...
  const editable = !finalized || allowFinalizedEditing;
  return <section className="participant-response-block"><header><p>{context.definition.is_required ? "Required response" : "Optional response"}</p><h2>{context.definition.label}</h2>{context.definition.instructions && <span>{context.definition.instructions}</span>}</header>{!editable ? <div className="participant-response-final"><p>{display || "No response provided."}</p><small>Response submitted</small></div> : <form><fieldset disabled={preview}>{responseKind === "short_text" || responseKind === "long_text" ? <><label htmlFor={`response-${context.definition.id}`} className="sr-only">{context.definition.label}</label><textarea id={`response-${context.definition.id}`} name="response" defaultValue={textValue} placeholder={String(configuration.placeholder ?? "")} maxLength={Number(configuration.maxLength) || (responseKind === "long_text" ? 8000 : 500)} rows={responseKind === "long_text" ? 10 : 4} required={context.definition.is_required}/></> : responseKind === "boolean" ? <label className="participant-choice"><input type="checkbox" name="response" value="true" defaultChecked={booleanValue} required={context.definition.is_required}/><span><strong>{String(configuration.affirmativeLabel)}</strong></span></label> : <fieldset><legend className="sr-only">{context.definition.label}</legend>{options.map((option) => <label className="participant-choice" key={option.key}><input type={responseKind === "single_select" ? "radio" : "checkbox"} name="response" value={option.key} defaultChecked={selected(data, option.key)} required={responseKind === "single_select" && context.definition.is_required}/><span><strong>{option.label}</strong>{option.description && <small>{option.description}</small>}</span></label>)}</fieldset>}</fieldset>{preview ? <small>Preview only · Responses are not saved</small> : allowFinalizedEditing && finalized ? <><div><button className="is-primary" formAction={finalizeResponseAction.bind(null, route.slug, route.moduleKey, route.lessonKey, route.sectionKey, route.blockKey, route.cohortId)} type="submit">Update response</button></div><small>Submitting replaces your previous response with this new answer.</small></> : <><div><button formAction={saveResponseDraftAction.bind(null, route.slug, route.moduleKey, route.lessonKey, route.sectionKey, route.blockKey, route.cohortId)} type="submit">Save draft</button><button className="is-primary" formAction={finalizeResponseAction.bind(null, route.slug, route.moduleKey, route.lessonKey, route.sectionKey, route.blockKey, route.cohortId)} type="submit">Submit response</button></div><small>Private to you. Submission is final.</small></>}</form>}</section>;
}
