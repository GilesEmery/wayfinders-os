...
import { accentInk, groupLabel, normalizeCourseConfiguration } from "@/lib/experiences/builder/course-configuration";
...
function ColumnContent({ column, responses, assets, route, preview, requirementsBypassed }: { column: BuilderColumn; responses: Readonly<Record<string, ParticipantResponseContext>>; assets: Readonly<Record<string, ResolvedAsset>>; route: ResponseRoute; preview: boolean; requirementsBypassed: boolean }) {
  const blocks = column.blocks.filter((block) => block.status === "active" && block.visibility === "visible");
  return <div className="participant-column-content">{blocks.length ? blocks.map((block) => <ParticipantBlockRenderer block={block} response={responses[block.id]} asset={assets[block.id]} route={route} preview={preview} requirementsBypassed={requirementsBypassed} key={block.id}/>) : <p className="participant-section-empty">No content is available in this Column yet.</p>}</div>;
}

function RuntimeColumn({ column, responses, assets, route, preview, requirementsBypassed }: { column: BuilderColumn; responses: Readonly<Record<string, ParticipantResponseContext>>; assets: Readonly<Record<string, ResolvedAsset>>; route: ResponseRoute; preview: boolean; requirementsBypassed: boolean }) {
  ...
  return <section className={className} style={style}>{collapsible ? <details open={!column.default_collapsed}><summary>{column.label || "Page content"}</summary><ColumnContent column={column} responses={responses} assets={assets} route={route} preview={preview} requirementsBypassed={requirementsBypassed}/></details> : <ColumnContent column={column} responses={responses} assets={assets} route={route} preview={preview} requirementsBypassed={requirementsBypassed}/>}</section>;
}

function SectionRuntime({ section, responses, assets, route, preview, requirementsBypassed }: { section: BuilderSection; responses: Readonly<Record<string, ParticipantResponseContext>>; assets: Readonly<Record<string, ResolvedAsset>>; route: ResponseRoute; preview: boolean; requirementsBypassed: boolean }) {
  ...
  return <div className={`participant-runtime-columns is-${section.layout.layout_mode}`}>{section.layout.columns.map((column) => <RuntimeColumn column={column} responses={responses} assets={assets} route={route} preview={preview} requirementsBypassed={requirementsBypassed} key={column.id}/>)}</div>;
}

export function ParticipantCourseRuntime({ structure, courseTemplate, themeConfiguration, coverUrl, logoUrl, headerLogoUrl, assets = { blocks: {}, heroes: {} }, companion, current, progress, responses, cohortId = null, requirementsBypassed = false, preview, navigationError, companionNotice, companionError }: { structure: BuilderCourseStructure; courseTemplate: CourseTemplate; themeConfiguration: unknown; coverUrl?: string | null; logoUrl?: string | null; headerLogoUrl?: string | null; assets?: { blocks: Readonly<Record<string, ResolvedAsset>>; heroes: Readonly<Record<string, ResolvedAsset>> }; companion: CompanionRuntimeData; current: CourseSectionLocation; progress: ParticipantProgressSnapshot; responses: Readonly<Record<string, ParticipantResponseContext>>; cohortId?: string | null; requirementsBypassed?: boolean; preview?: { experienceId: string; versionId: string; cohortId?: string | null }; navigationError?: string; companionNotice?: string; companionError?: string }) {
  ...
  const completedGroups = structure.modules.filter((module) => progress.modules[module.id] === "completed").length;
  const completedGroupLabel = groupLabel(configuration.terminology.group_label, completedGroups !== 1);
  ...
  return <div className={`participant-course ${themeClasses} course-header-${configuration.appearance.header_treatment} course-reading-${configuration.appearance.reading_width}`} style={appearanceStyle}>
    ...
    {!preview && requirementsBypassed && <div className="participant-preview-banner is-live-override" role="status">Admin / leader access · Course requirements are optional · Responses still save normally</div>}
    ...
    <div className={`participant-course-layout is-${courseTemplate.key}`}><main>
      ...
      <header className="participant-section-header"><p>{current.moduleTitle} / {current.lessonTitle}</p><h1>{current.section.title}</h1>{current.section.description && <span>{current.section.description}</span>}</header>
      ...
      <SectionRuntime section={current.section} responses={responses} assets={assets.blocks} route={{ slug: structure.experience.slug, moduleKey: current.moduleKey, lessonKey: current.lessonKey, sectionKey: current.section.section_key, cohortId }} preview={Boolean(preview)} requirementsBypassed={requirementsBypassed}/>
      ...
      <nav className="participant-section-pagination" aria-label="Lesson navigation">{previous ? <Link href={href(previous)}><small>Previous</small><strong>{previous.section.title}</strong></Link> : <span/>}{next ? preview || requirementsBypassed ? <Link href={href(next)}><small>Continue to next</small><strong>{next.section.title}</strong></Link> : <form action={navigateForwardAction.bind(null, structure.experience.slug, current.moduleKey, current.lessonKey, current.section.section_key, next.moduleKey, next.lessonKey, next.section.section_key, cohortId)}><button type="submit"><small>Continue to next</small><strong>{next.section.title}</strong></button></form> : preview ? <div><small>Preview end</small><strong>You have reached the final Page.</strong></div> : requirementsBypassed ? <div><small>Admin / leader access</small><strong>You have reached the final Page.</strong></div> : progress.sections[current.section.id] === "completed" && progress.experience.status === "completed" ? <div><small>Course complete</small><strong>Your progress has been saved.</strong></div> : <form action={finishCourseAction.bind(null, structure.experience.slug, current.moduleKey, current.lessonKey, current.section.section_key, cohortId)}><button type="submit"><small>Final Page</small><strong>Complete Course</strong></button></form>}</nav>
    </main><aside className="participant-course-outline"><details open><summary>Course Outline</summary><div className="participant-outline-identity"><strong>{structure.version.title || structure.experience.name}</strong>{structure.version.description && <span>{structure.version.description}</span>}<div className="participant-progress-summary"><span>{progress.experience.requiredPercent}% complete · {completedGroups} {completedGroupLabel} completed</span><i aria-label="Course progress" aria-valuemax={100} aria-valuemin={0} aria-valuenow={progress.experience.requiredPercent} role="progressbar"><span style={{ width: `${progress.experience.requiredPercent}%` }}/></i></div></div><CourseNavigator structure={structure} current={current} progress={progress} hrefFor={href} completeOnForward={!preview && !requirementsBypassed} cohortId={cohortId}/></details></aside>{courseTemplate.shell.includes("group_companion") && <ParticipantCompanion companion={companion} current={current} preview={Boolean(preview)} notice={companionNotice} error={companionError} route={{ slug: structure.experience.slug, moduleKey: current.moduleKey, lessonKey: current.lessonKey, sectionKey: current.section.section_key, cohortId }}/>}</div>
  </div>;
}
