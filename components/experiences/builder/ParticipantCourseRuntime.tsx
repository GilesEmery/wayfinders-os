...
export function ParticipantCourseRuntime({ structure, courseTemplate, themeConfiguration, coverUrl, logoUrl, headerLogoUrl, assets = { blocks: {}, heroes: {} }, companion, current, progress, responses, preview, navigationError, companionNotice, companionError }: { structure: BuilderCourseStructure; courseTemplate: CourseTemplate; themeConfiguration: unknown; coverUrl?: string | null; logoUrl?: string | null; headerLogoUrl?: string | null; assets?: { blocks: Readonly<Record<string, ResolvedAsset>>; heroes: Readonly<Record<string, ResolvedAsset>> }; companion: CompanionRuntimeData; current: CourseSectionLocation; progress: ParticipantProgressSnapshot; responses: Readonly<Record<string, ParticipantResponseContext>>; preview?: { experienceId: string; versionId: string }; navigationError?: string; companionNotice?: string; companionError?: string }) {
  ...
  return <div className={`participant-course ${themeClasses} course-header-${configuration.appearance.header_treatment} course-reading-${configuration.appearance.reading_width}`} style={appearanceStyle}>
    ...
    </main><aside className="participant-course-outline"><details open><summary>Course Outline</summary><div className="participant-outline-identity"><strong>{structure.version.title || structure.experience.name}</strong>{structure.version.description && <span>{structure.version.description}</span>}<div className="participant-progress-summary"><span>{progress.experience.requiredPercent}% complete · {progress.experience.requiredCompleted} of {progress.experience.requiredTotal} required Pages</span><i aria-label="Required Page progress" aria-valuemax={100} aria-valuemin={0} aria-valuenow={progress.experience.requiredPercent} role="progressbar"><span style={{ width: `${progress.experience.requiredPercent}%` }}/></i></div></div><CourseNavigator structure={structure} current={current} progress={progress} hrefFor={href} completeOnForward={!preview}/></details></aside>{courseTemplate.shell.includes("group_companion") && <ParticipantCompanion companion={companion} current={current} mode={configuration.companion_mode} preview={Boolean(preview)} notice={companionNotice} error={companionError} route={{ slug: structure.experience.slug, moduleKey: current.moduleKey, lessonKey: current.lessonKey, sectionKey: current.section.section_key }}/>}</div>
  </div>;
}
