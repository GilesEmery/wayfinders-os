...
export function ParticipantCourseRuntime({ structure, courseTemplate, themeConfiguration, coverUrl, logoUrl, assets = { blocks: {}, heroes: {} }, current, progress, responses, preview, navigationError }: { structure: BuilderCourseStructure; courseTemplate: CourseTemplate; themeConfiguration: unknown; coverUrl?: string | null; logoUrl?: string | null; assets?: { blocks: Readonly<Record<string, ResolvedAsset>>; heroes: Readonly<Record<string, ResolvedAsset>> }; current: CourseSectionLocation; progress: ParticipantProgressSnapshot; responses: Readonly<Record<string, ParticipantResponseContext>>; preview?: { experienceId: string; versionId: string }; navigationError?: string }) {
  ...
  return <div className={`participant-course ${themeClasses} course-header-${configuration.appearance.header_treatment} course-reading-${configuration.appearance.reading_width}`} style={appearanceStyle}>
    ...
    <header className="participant-course-header">{configuration.appearance.header_treatment === "image" && coverUrl && <Image className="participant-course-cover" src={coverUrl} alt="" fill unoptimized sizes="100vw"/>}<div><p>PurposeOS</p><h1>{structure.version.title || structure.experience.name}</h1></div>{logoUrl && <Image className="participant-course-logo" src={logoUrl} alt={`${structure.version.title || structure.experience.name} logo`} width={220} height={44} unoptimized/>}{preview ? <Link href={`/admin/trainings/${preview.experienceId}/versions/${preview.versionId}`}>Back to Builder ↗</Link> : <Link href="/dashboard">My Learning ↗</Link>}</header>
    <div className={`participant-course-layout is-${courseTemplate.key}`}><main>
      ...
      <SectionRuntime section={current.section} responses={responses} assets={assets.blocks} route={{ slug: structure.experience.slug, moduleKey: current.moduleKey, lessonKey: current.lessonKey, sectionKey: current.section.section_key }} preview={Boolean(preview)}/>
