...
export function ParticipantBlockRenderer({ block, response, route, preview = false }: { block: BuilderContentBlock; response?: ParticipantResponseContext; route: { slug: string; moduleKey: string; lessonKey: string; sectionKey: string }; preview?: boolean }) {
  ...
  if (definition.previewKey === "media") return <ParticipantMediaBlock kind={block.block_type as "video" | "image" | "document" | "download" | "external_link"} config={configuration}/>;
