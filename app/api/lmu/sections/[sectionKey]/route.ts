...
export async function PUT(request: NextRequest, context: Context) {
  try {
    ...
    const [responseQuery, progressQuery, assessmentQuery] = await Promise.all([
      admin.from("lmu_responses").upsert({ assessment_id: session.assessment_id, section_key: sectionKey, response_data: responseData as Json }, { onConflict: "assessment_id,section_key" }),
...
export async function DELETE(request: NextRequest, context: Context) {
  try {
    ...
    const [responseQuery, progressQuery, resultQuery] = await Promise.all([
      admin.from("lmu_responses").delete().eq("assessment_id", session.assessment_id).eq("section_key", sectionKey),
      admin.from("lmu_section_progress").delete().eq("assessment_id", session.assessment_id).eq("section_key", sectionKey),
      admin.from("lmu_results").delete().eq("assessment_id", session.assessment_id).eq("section_key", sectionKey),
    ]);
