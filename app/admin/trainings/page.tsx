...
export default async function Page({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <AdminShell admin={admin}>
    ...
    <div className="admin-table-wrap"><table><thead><tr><th>Experience</th><th>Type</th><th>Delivery</th><th>Experience lifecycle</th><th>Visibility</th><th>Current Published Version</th><th>Draft Versions</th><th>Owner</th><th>Updated</th></tr></thead><tbody>{rows.map(item => <tr key={item.id}><td><Link href={`/admin/trainings/${item.id}`}><strong>{item.name}</strong><br/><small>{item.slug}</small></Link></td><td>{humanize(item.experience_type)}</td><td>{humanize(item.delivery_mode)}</td><td><span className={`admin-status is-${item.status}`}>{humanize(item.status)}</span></td><td>{humanize(item.visibility)}</td><td>{item.currentVersion ? `${item.currentVersion.version_label} · Published` : "No Published Version"}</td><td>{item.draftVersionCount ? `${item.draftVersionCount} Draft${item.draftVersionCount === 1 ? "" : "s"}` : "No Draft Version"}</td><td>{item.ownerName ?? "Platform"}</td><td>{formatDate(item.updated_at)}</td></tr>)}{!rows.length && <tr><td colSpan={9}>No Experiences match this view.</td></tr>}</tbody></table></div>
  </AdminShell>;
}
