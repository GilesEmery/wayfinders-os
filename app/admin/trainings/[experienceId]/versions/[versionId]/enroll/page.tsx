...
export default async function EnrollmentPage({ params, searchParams }: { params: Promise<{ experienceId: string; versionId: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <AdminShell admin={admin}>
    ...
    <section className="admin-panel"><p className="admin-kicker">Current learners</p><h2>Published Course enrollments</h2><div className="admin-table-wrap"><table><thead><tr><th>Wayfinder</th><th>Account</th><th>Enrollment</th><th>Enrolled</th></tr></thead><tbody>{(enrollments.data ?? []).map((item) => { const person = people.get(item.participant_id); return <tr key={item.id}><td>{person?.full_name || person?.email || item.participant_id}</td><td>{person?.auth_user_id ? "Account active" : "Not activated"}</td><td>{item.status}</td><td>{new Date(item.enrolled_at).toLocaleDateString()}</td></tr>; })}{!enrollments.data?.length && <tr><td colSpan={4}>No learners are enrolled in this Published Course yet.</td></tr>}</tbody></table></div></section>
  </AdminShell>;
}
