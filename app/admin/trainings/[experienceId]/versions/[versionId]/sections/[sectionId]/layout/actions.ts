...
import { createBlock, deleteBlock, duplicateBlock, moveBlock, reorderBlock, updateBlock } from "@/lib/experiences/admin/block-mutations";
...
async function runBlock(experienceId: string, versionId: string, sectionId: string, operation: () => Promise<void>, saved: string) {
  ...
  const destination = (message?: { error?: string; saved?: string }) => {
  ...
  try { await operation(); } catch (error) { redirect(destination({ error: error instanceof Error ? error.message : "Unable to update Content." })); }
  revalidatePath(path(experienceId, versionId, sectionId));
...
export async function updateBlockAction(experienceId: string, versionId: string, sectionId: string, blockId: string, form: FormData) { await runBlock(experienceId, versionId, sectionId, () => updateBlock(experienceId, versionId, sectionId, blockId, form), "Content saved."); }
