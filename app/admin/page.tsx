...
export default async function Page() {
  ...
  return <AdminShell admin={identity}><AdminPageHeader eyebrow="Purpose OS overview" title="Dashboard" description="A clear operating view across Wayfinders, experiences, organizations, engagement, and insights."/>
    ...
    <div className="admin-dashboard-columns"><section className="admin-panel"><p className="admin-kicker">Activity</p><h2>Recent activity</h2>{activity?.length?<ul className="admin-activity-list">{activity.map(row=><li key={row.id}><div><strong>{humanize(row.action)}</strong><span>{row.admin_email}{row.entity_type?` · ${humanize(row.entity_type)}`:""}</span></div><time>{formatDate(row.created_at)}</time></li>)}</ul>:<p className="admin-muted">No recorded admin activity yet.</p>}</section><section className="admin-panel"><p className="admin-kicker">Journey</p><h2>Quick access</h2><div className="admin-quick-actions"><Link href="/admin/users">View Wayfinders</Link><Link href="/admin/hubs">View Hubs</Link><Link href="/admin/partners">View Partners</Link><Link href="/admin/assessments/life-mapping-u">View LMU results</Link><Link href="/admin/trainings">Experience registry</Link><Link href="/admin/analytics">View analytics</Link></div></section></div>
  </AdminShell>;
}
