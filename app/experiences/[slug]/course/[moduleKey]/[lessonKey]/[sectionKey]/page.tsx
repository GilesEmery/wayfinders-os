...
export default async function ParticipantCoursePage({ params, searchParams }: { params: Params; searchParams: SearchParams }) {
  ...
  return <PlatformShell contextTitle={result.structure.version.title || result.structure.experience.name}><ParticipantCourseRuntime structure={result.structure} courseTemplate={result.courseTemplate} themeConfiguration={result.themeConfiguration} coverUrl={result.coverUrl} logoUrl={result.logoUrl} headerLogoUrl={result.headerLogoUrl} assets={result.assets} companion={result.companion} current={current} progress={result.progress} responses={result.responses} cohortId={result.cohortId} requirementsBypassed={result.requirementsBypassed} navigationError={progressError} companionError={companionError} companionNotice={companionSaved === "1" ? "Note saved" : companionSaved}/></PlatformShell>;
}
