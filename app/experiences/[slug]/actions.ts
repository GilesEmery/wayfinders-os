...
export async function selfEnrollAction(slug: string) {
  ...
  const existing = await db.from("experience_enrollments").select("id,experience_version_id").eq("participant_id", profile.participant.id).eq("experience_id", target.id).is("cohort_id", null).in("status", ACTIVE_ENROLLMENTS);
