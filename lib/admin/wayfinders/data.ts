...
export async function loadWayfinderWorkspace(participantId: string) {
  ...
  const [adminMember, roles, entitlementCatalog, roleEntitlementDefaults, entitlementOverrides] = authId ? await Promise.all([
    db.from("admin_members").select("role,status").eq("auth_user_id", authId).maybeSingle(),
    db.from("platform_role_assignments").select("id,role,scope_type,scope_id,status,created_at").eq("auth_user_id", authId).eq("status", "active"),
    db.from("authorization_entitlements").select("entitlement_key,display_name,description").order("display_name"),
    db.from("authorization_role_entitlements").select("role_key,entitlement_key"),
    db.from("authorization_entitlement_overrides").select("entitlement_key,effect").eq("auth_user_id", authId),
  ]) : [{ data: null }, { data: [] }, { data: [] }, { data: [] }, { data: [] }];
  return { participant: participant.data, accountLinkingIssue: (emailMatches.count ?? 0) > 1, assessments: assessments.data ?? [], orgMemberships: orgMemberships.data ?? [], hubMemberships: hubMemberships.data ?? [], cohortMemberships: cohortMemberships.data ?? [], participantTags: participantTags.data ?? [], enrollments: enrollments.data ?? [], progress: progress.data ?? [], entitlements: entitlements.data ?? [], assignments: assignments.data ?? [], preferences: preferences.data, notes: notes.data ?? [], auditRows: auditRows.data ?? [], orgs: orgs.data ?? [], hubs: hubs.data ?? [], cohorts: cohorts.data ?? [], tags: tags.data ?? [], experiences: experiences.data ?? [], offerings: offerings.data ?? [], adminMember: adminMember.data, roles: roles.data ?? [], entitlementCatalog: entitlementCatalog.data ?? [], roleEntitlementDefaults: roleEntitlementDefaults.data ?? [], entitlementOverrides: entitlementOverrides.data ?? [] };
}
