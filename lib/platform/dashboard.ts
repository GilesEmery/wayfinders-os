...
export async function getWayfinderDashboard() {
  ...
  const [sectionProgress, enrollments, progress, organizations, hubs, cohorts, tags, roles, adminAccess] = await Promise.all([
    ...
    db.from("admin_members").select("role,status").eq("auth_user_id", user.id).eq("status", "active").in("role", ["admin", "super_admin"]).maybeSingle(),
  ]);
  ...
  return {
    ...
    roles: roles.data ?? [],
    adminAccess: adminAccess.data ?? null,
  } as const;
}
