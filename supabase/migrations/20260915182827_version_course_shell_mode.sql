-- Version-owned course shell selection. Existing and new Versions default to Standard.
alter table public.experience_versions
  add column shell_mode text not null default 'standard'
  constraint experience_versions_shell_mode_check check (shell_mode in ('standard', 'enhanced'));

comment on column public.experience_versions.shell_mode is
  'Release-pinned standard Navigator + Content or enhanced Navigator + Content + Companion shell. Custom-code adapters may ignore it.';

-- Keep the existing atomic deep clone exactly as applied, adding only shell_mode
-- to the Version metadata copied into the successor Draft.
...
begin
  ...
  insert into public.experience_versions (experience_id, version_label, status, title, description,
    based_on_version_id, release_type, theme_id, shell_mode, created_by)
  values (p_experience_id, p_version_label, 'draft', v_source.title, v_source.description,
    p_source_version_id, v_source.release_type, v_source.theme_id, v_source.shell_mode, p_actor_id)
