-- Version-owned course terminology and curated appearance are release-pinned.
alter table public.experience_versions
  add column course_configuration jsonb not null default '{}'::jsonb
  constraint experience_versions_course_configuration_object_check
    check (pg_catalog.jsonb_typeof(course_configuration) = 'object');

comment on column public.experience_versions.course_configuration is
  'Release-pinned terminology and curated appearance choices; shared Themes remain separate.';

-- Preserve the applied deep-clone implementation, copying only the new Version field.
