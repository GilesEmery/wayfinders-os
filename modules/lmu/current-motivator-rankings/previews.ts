...
export function getMotivatorPreview(definition: MotivatorDefinition, progress: ParticipantModuleProgress[]): MotivatorPreview {
  ...
  if(definition.id==="salary") { const response=source.responses as unknown as SalaryResponse; const floor=response.financialFloor; const goal=response.fiveYearGoal; const perspective=compensationPerspective(response.tradeoffScenarios??[]); const context=response.tradeoffVersion?perspective.summary:undefined; return {complete:true,shortPreview:floor&&goal?[`${formatAmount(floor,response.currency)} → ${formatAmount(goal,response.currency)} · ${response.compensationPeriod}`,context].filter(Boolean).join(" — "):"Salary range complete",expandedGroups:context?[{label:"Compensation Perspective",items:[context,...(response.compensationReflection?[response.compensationReflection]:[])]}]:[],salaryRange:response}; }
