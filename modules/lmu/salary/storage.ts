...
export function isSalaryComplete(response: SalaryResponse) {
  const foundation = Boolean(response.currency && response.financialFloor && response.financialFloor > 0 && response.fiveYearGoal && response.fiveYearGoal > 0 && response.significantChange && response.advisorRelationship && response.finalizedAt);
  if (!foundation) return false;
  if (!response.tradeoffVersion) return true;
  return Boolean(response.tradeoffScenarios?.length && response.tradeoffScenarios.every((scenario) => scenario.choice && scenario.difficulty));
}
...
export function saveSalaryResponse(response: SalaryResponse, complete = false) {
  ...
  saveModuleProgress(LMU_ORIGINAL_EXPERIENCE_ID, {
    ...
    derivedResults: { currency: response.currency, compensationPeriod: response.compensationPeriod, financialFloor: response.financialFloor, fiveYearGoal: response.fiveYearGoal, tradeoffScenarios: response.tradeoffScenarios, compensationReflection: response.compensationReflection },
